...
def index():
    ...
    ###searching by genre, actor, rating###
    ...
    # list of (a_id, a_firstname, a_lastname)
    actors = cur.fetchall()   
    ...
    if request.method == "POST":
        ...
        if "submit_genre" in request.form:
            ...
            if selected_actor_id:
                ...
                cur.execute(
                    """
                    SELECT DISTINCT m.m_id, m.m_title, m.m_releaseYear, m.m_rating
                    FROM tables.movie AS m
                ...
                cur.execute(
                    """
                    SELECT DISTINCT s.s_id, s.s_title, s.s_releaseYear, s.s_duration, s.s_rating
                    FROM tables.show AS s
                    JOIN tables.acted_in AS ai
                      ON s.s_id = ai.s_id
                    WHERE ai.a_id = %s
            ...
            if selected_min_rating and selected_max_rating:
                ...
                cur.execute(
                    """
                    SELECT m.m_id, m.m_title, m.m_releaseYear, m.m_rating
                    FROM tables.movie AS m
